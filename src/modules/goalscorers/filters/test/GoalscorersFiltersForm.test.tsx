...
const teamOptions = [{ label: 'All Teams', value: 'all' }];

const defaultValues: TGoalscorersFilters = { competitionId: 'all', teamId: 'all' };
  ...
  return render(
    <TestWrapper>
      <GoalscorersContext.Provider
        value={{ filters: { competitionId: 'all', teamId: 'all' }, setFilters: vi.fn() }}
      >
...
describe('GoalscorersFiltersForm', () => {
  ...
  it('calls onSubmit with the current filter values when Apply is clicked', async () => {
    ...
    await user.click(screen.getByText('FILTERS.ALL_COMPETITIONS'));
    await user.click(screen.getByText('FILTERS.APPLY'));

    await waitFor(() => {
      expect(onSubmit).toHaveBeenCalledWith({ competitionId: 'all', teamId: 'all' });
    });
  });
