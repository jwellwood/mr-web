...
function renderWithContext(competitionId: string, teamId: string = 'all') {
  return render(
    <TestWrapper>
      <GoalscorersContext.Provider
        value={{ filters: { competitionId, teamId }, setFilters: vi.fn() }}
      >
        <GoalscorersFiltersDisplay competitionOptions={competitionOptions} teamOptions={[]} />
      </GoalscorersContext.Provider>
    </TestWrapper>
  );
}
