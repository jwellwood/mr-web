...
export const rows = (stats?: IPlayerStats, loading?: boolean) => {
  const {
    apps,
    goals,
    assists,
  ...
  return [
    {
      ...
      average: loading ? <StatSkeleton /> : getAvg(goals, apps) || 0,
    },
    {
      icon: <StatIcon icon={STAT_ICONS.ASSIST} />,
      label: 'Assists',
      value: loading ? <StatSkeleton /> : assists,
      average: loading ? <StatSkeleton /> : getAvg(assists, apps) || 0,
    },
    {
      icon: <StatIcon icon={STAT_ICONS.MVP} />,
      label: 'MVP',
      value: loading ? <StatSkeleton /> : mvp,
      average: loading ? <StatSkeleton /> : getAvg(mvp, apps) || 0,
    },
    {
      ...
      average: loading ? (
        ...
        {
          value: getPercentage(pensScored, pensScored + pensMissed) || 0,
          isPercentage: true,
        }
      ),
    },
    {
      ...
      average: loading ? (
        ...
        {
          value: getPercentage(pensMissed, pensScored + pensMissed) || 0,
          isPercentage: true,
        }
      ),
    },
    {
      ...
      average: loading ? <StatSkeleton /> : getAvg(ownGoals, apps) || 0,
    },
    {
      icon: <StatIcon icon={STAT_ICONS.CONCEDED} />,
      label: 'Conceded',
      value: loading ? <StatSkeleton /> : conceded,
      average: loading ? <StatSkeleton /> : getAvg(conceded, apps) || 0,
    },
    {
