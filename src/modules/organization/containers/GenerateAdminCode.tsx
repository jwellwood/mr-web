...
export default function GenerateAdminCode() {
  ...
  const onSubmit = async () => {
    try {
      ...
      dispatch(showAlert({ text: t('ALERTS.GENERATE_ADMIN_CODE.SUCCESS'), type: 'success' }));
    } catch (err) {
      console.error(err);
      dispatch(showAlert({ text: t('ALERTS.GENERATE_ADMIN_CODE.ERROR'), type: 'error' }));
    }
  };
