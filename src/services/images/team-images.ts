import axios from 'axios';
import { API_PATH, ROOT_URL } from '../../constants';
import { authStorage } from '../../utils';

const api = {
  UPLOAD_TEAM_BADGE_ROUTE: `${ROOT_URL}${API_PATH.ADMIN}/upload_team_badge`,
  REMOVE_TEAM_BADGE_ROUTE: `${ROOT_URL}${API_PATH.ADMIN}/remove_team_badge?public_id=`,
};

const getAuthHeaders = () => ({
  authorization: authStorage.getToken() ?? '',
});

export const uploadTeamBadge = (dataToSubmit: FormData) => {
  const route = api.UPLOAD_TEAM_BADGE_ROUTE;
  const request = axios
    .post(route, dataToSubmit, { headers: getAuthHeaders() })
    .then(res => res.data);
  return request;
};

export const removeTeamBadge = (public_id: string) => {
  const route = `${api.REMOVE_TEAM_BADGE_ROUTE}${encodeURIComponent(public_id)}`;
  const request = axios.delete(route, { headers: getAuthHeaders() }).then(res => res.data);
  return request;
};
